...
1) added a global boolean variable 'my_debug' with which 
   easily all debug print statements can be activated or
   de-activated.

2) adding a class named 'SAA'. SAA stands for 'Spotify
   Album Art'. This class has been created to store
...
3) added the use of a textfile with the name 'secrets.h'.
...
4) Changing the use of SPIFFS file system to use the
...
5) moving some functionality that were in the main 
...
7) Added functions to display the list of
   artists in case the track or album has more than
   one artist;

8) Added handling of letters in the ASCII extended Latin-1 range.
   ...
   1) the string.length() function counts the 'hidden' first
   ...
   2) the TFT_eSPI library does not display Latin-1 group
   capital letters. We have to convert them to small letters.

   Workaround:

   1a) In the function convertUnicode() :
   ...
   1b) Below the part of ConvUpperToLower() that filters for and handles
   Latin-1 group characters:

   1b.1) If it is the first character of the string 
   ...
   1b.2) if it is any other letter in the string:
   ...
   a) the ASCII extended Latin-1 group of letters; 
   b) the handling of some long album names like:
...
9) Using another font by downloading, converting and flashing a font file.
...
I also decided to comment-out some precompiler conditions, e.g.Ç
